...
@app.route("/Obi", methods=["GET", "POST"])
def get_obi():
    if request.method == "POST":
        print("OBI POST REQUEST")
        print(f"Received data: {request.data}")
        print(f"Json data: {request.json}")
        # checking for request.form["token"] causes a CORS error, so we will just check for the token in the json body
        # I think it's cause sending things as a JSON type makes .form work differently
        try:
    ...
    if os.path.isfile("Obi.png"):
        import base64
        with open("Obi.png", "rb") as img_file:
